...
import { planScenesForProject, toSceneRouteError } from "../services/scenePlanningService.js";
...
router.post("/plan", async (request, response) => {
  try {
    ...
    const lesson = await planScenesForProject(lessonId);
    response.json({ lesson });
  } catch (error) {
    const routeError = toSceneRouteError(error);
